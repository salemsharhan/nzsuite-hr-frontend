...
export async function sendWhatsAppApprovalPoll(
  whatsTaskUrl: string,
  integrationSecret: string,
  workspaceUserId: string,
  recipientJid: string,
...
): Promise<{ ok: boolean; error?: string; poll_message_id?: string | null }> {
  const jid = recipientJid.trim().toLowerCase()
  if (!jid) return { ok: false, error: 'Missing recipient JID' }

  const res = await fetch(whatsTaskFunctionUrl(whatsTaskUrl, 'hr-send-approval-poll'), {
    method: 'POST',
    headers: whatsTaskFetchHeaders(integrationSecret),
    body: JSON.stringify({
      workspace_user_id: workspaceUserId,
      recipient_wa_jid: jid,
      recipient_phone_e164: recipientPhone?.trim() || undefined,
      poll_text: pollText.slice(0, 900),
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) {
    return { ok: false, error: (data as { error?: string }).error ?? `Whats-Task poll error (${res.status})` }
  }
  return {
    ok: true,
    poll_message_id: (data as { poll_message_id?: string | null }).poll_message_id ?? null,
  }
}

export function buildGmPayrollApprovalPollText(
  periodLabelAr: string,
  gmName?: string,
  attachmentLink?: string,
): string {
  const greeting = gmName?.trim() ? `السلام عليكم ${gmName.trim()}،` : 'السلام عليكم ورحمة الله،'
  const parts = [
    greeting,
    '',
    `تم رفع كشف الرواتب لشهر ${periodLabelAr} لاعتمادكم.`,
    '',
  ]
  if (attachmentLink?.trim()) {
    parts.push('رابط كشف الرواتب:', attachmentLink.trim(), '')
  }
  parts.push(
    'يرجى التكرم باختيار:',
    '✅ موافقة',
...
const GM_APPROVAL_POLL_OPTIONS = ['✅ موافقة', '❌ رفض', '💬 تعليق']

export { GM_APPROVAL_POLL_OPTIONS }
